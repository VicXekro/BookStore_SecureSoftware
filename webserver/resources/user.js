...
router.post('/register', (req, res) => {
    ...
    connect.querysql(`SELECT * FROM users WHERE username = '${username}'`).then(result => {
        ...
        const password = crypto.createCipher('aes192', req.params.password);
        connect.querysql(`INSERT INTO users (username, password, phone, name, email) VALUES ('${username}', '${password}', ${req.params.phonenumber}, '${req.params.name}', '${req.params.email}');`).then(() => {
...
router.post('/billing', (reg, res) => {
    ...
    let card = crypto.createCipher('aes192', req.params.card);
    connect.querysql(`UPDATE users SET address = '${address}', card = ${card} WHERE username = '${req.params.username}'`).then(() => {
